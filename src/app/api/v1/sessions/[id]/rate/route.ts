...
/** GET /api/v1/sessions/:id/rate — Get ratings */
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  return successResponse(await getRatings('session', id, limit, offset))
}

/** POST /api/v1/sessions/:id/rate — Rate a session */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  return successResponse(await upsertRating('session', id, auth.user.id, parsed.data), 201)
}

/** PATCH /api/v1/sessions/:id/rate — Creator responds to a rating */
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  const result = await addCreatorResponse(ratingId, auth.user.id, 'session', parsed.data.creatorResponse)
