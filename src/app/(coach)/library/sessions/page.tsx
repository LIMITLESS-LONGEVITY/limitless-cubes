...
export default function LibrarySessionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['library-sessions', params],
    queryFn: () => sessionApi.list(params),
  })

  const likeMutation = useMutation({
    mutationFn: (id: string) => sessionApi.like(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['library-sessions'] }),
  })
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--surface-app)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-neutral-100">Session Library</h1>
          <p className="text-sm text-neutral-400 mt-1">Browse and manage training sessions</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.data.map((session) => (
                <SessionCard
                  key={session.id}
                  session={session}
                  onNavigate={() => router.push(`/library/sessions/${session.id}`)}
                  onLike={() => likeMutation.mutate(session.id)}
                  onFork={() => forkMutation.mutate(session.id)}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: phaseGradient(session.id) }}
      >
        ...
        <Layers size={40} className="text-white/10 absolute bottom-3 right-3" />
      </div>
      ...
      <div className="p-4 flex flex-col gap-2.5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-neutral-400 truncate">{session.creator.fullName}</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: '#222' }}>
          <div className="flex items-center gap-3 text-xs text-neutral-500">
            <span className="flex items-center gap-1"><Heart size={12} /> {session._count.likes}</span>
            <span className="flex items-center gap-1"><GitFork size={12} /> {session._count.forks}</span>
          </div>
