...
export default function LibraryProgramsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['library-programs', params],
    queryFn: () => programApi.list(params),
  })

  const likeMutation = useMutation({
    mutationFn: (id: string) => programApi.like(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['library-programs'] }),
  })
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--surface-app)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-neutral-100">Program Library</h1>
          <p className="text-sm text-neutral-400 mt-1">Browse and manage training programs</p>
        </div>
        ...
        <div className="flex flex-col gap-4 mb-6">
            ...
            <div className="flex gap-3">
              {difficultyLevels && difficultyLevels.length > 0 && (
                <select
                  ...
                  onChange={(e) => resetAndSet(setDifficultyLevelId, e.target.value)}
                  className="rounded-lg px-3 py-2.5 text-sm text-neutral-200 outline-none"
                  style={{ background: 'var(--surface-card)', border: '1px solid #333' }}
                >
              ...
              <div className="relative">
                <select
                  ...
                  className="rounded-lg pl-3 pr-8 py-2.5 text-sm text-neutral-200 outline-none appearance-none"
                  style={{ background: 'var(--surface-card)', border: '1px solid #333' }}
                >
          ...
          {domains && domains.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => resetAndSet(setDomainId, '')}
              ...
              {domains.map((d) => (
                <button
                  key={d.id}
                  onClick={() => resetAndSet(setDomainId, d.id)}
                  className="px-3 py-1.5 text-xs font-medium rounded-full transition-colors"
                  style={{
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.data.map((program) => (
                <ProgramCard
                  key={program.id}
                  program={program}
                  onNavigate={() => router.push(`/library/programs/${program.id}`)}
                  onLike={() => likeMutation.mutate(program.id)}
                  onFork={() => forkMutation.mutate(program.id)}
            ...
            {hasMore && (
              <div className="flex justify-center mt-8">
                <button
                  ...
                  style={{ background: 'var(--surface-card)', color: '#e5e5e5', border: '1px solid #333' }}
                >
                  Load more
                </button>
              </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: phaseGradient(program.id) }}
      >
        ...
        <CalendarDays size={40} className="text-white/10 absolute bottom-3 right-3" />
      </div>
      ...
      <div className="p-4 flex flex-col gap-2.5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-neutral-400 truncate">{program.creator.fullName}</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: '#222' }}>
          <div className="flex items-center gap-3 text-xs text-neutral-500">
            <span className="flex items-center gap-1"><Heart size={12} /> {program._count.likes}</span>
            <span className="flex items-center gap-1"><GitFork size={12} /> {program._count.forks}</span>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              style={{ color: 'var(--accent-light)', background: 'var(--accent-subtle)' }}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="rounded-lg h-80 animate-pulse" style={{ background: 'var(--surface-card)', border: '1px solid #222' }} />
      ))}
    </div>
  )
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm text-neutral-400">No programs found</p>
